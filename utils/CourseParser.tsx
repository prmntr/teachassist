...
// Distinct semester value for summer-school courses. 1/2 are the regular
// semesters; 0 is a full-year / non-semestered course. Summer school is its own
// short term, so it gets its own bucket for grouping, labelling and averaging.
...
function parseCourseInfo(text: string, course: Partial<Course>): void {
  ...
  if (courseMatch) {
    let courseCode = courseMatch[1]?.trim();
    const courseName = courseMatch[2]?.trim();

    // Summer school / cross-school reports prefix the board + school name, e.g.
    // "YRDSB - Richmond Hill Secondary Summer School - GLC2O1-3". The real course
    // code is the final " - "-delimited segment. Regular reports have no such
    // prefix, and section suffixes like "GLC2O1-3" use no spaces around the
    // hyphen, so this never touches a normal code.
    if (courseCode?.includes(" - ")) {
      courseCode = courseCode.slice(courseCode.lastIndexOf(" - ") + 3).trim();
    }
...
function determineSemester(startDate: string, endDate: string): number {
  ...
  // Summer school: short sessions whose start AND end both land in the summer
  // months (June–August). Regular semesters start in Sept (1) or Feb (2) and
  // full-year courses start in Sept, so none of them have both endpoints in this
  // window — checking it first is unambiguous. e.g. YRDSB summer runs late June
  // through July.
  const isSummer =
    startMonth >= 5 && startMonth <= 7 && endMonth >= 5 && endMonth <= 7;
  if (isSummer) return SUMMER_SEMESTER;
