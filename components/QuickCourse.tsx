...
export const CourseInfoBox = ({
  ...
  const { activeTone, isDark, themePresetId } = useTheme();
  ...
  const displayCourse = useMemo((): DisplayCourse => {
    ...
    const midtermMark = getVisualMark(
      course.midtermMark ?? null,
      shouldForceVisualHundreds,
    );
    const finalMark = getVisualMark(
      course.finalMark ?? null,
      shouldForceVisualHundreds,
    );
    const resolvedCurrentMark = getVisualMark(
  ...
  }, [course, overrideCourseMark, shouldForceVisualHundreds]);
      ...
      <View className="">
        ...
        >
          ...
          {showStaleIndicator && (
            ...
            >
              <View
                className={`bg-caution rounded-full pl-3 pr-2 pt-2 pb-3 `}
              ></View>
            </TouchableOpacity>
          )}
          <View style={{ zIndex: 1 }}>
              ...
              <View className="flex-row items-center mt-3">
                {displayCourse?.midtermMark && (
                  ...
                  >
                    {showMidtermMark ? (
                      <View
                        className={`bg-baccent/85 rounded-xl px-3 py-1`}
                        style={{ alignSelf: "flex-start" }}
                      >
                        <Text
                          className={`${isDark ? "text-appblack" : "text-appwhite"} text-sm font-medium`}
                        >
                      ...
                      <View
                        className={`${isDark ? "bg-dark4" : "bg-light4"} rounded-xl px-3 py-1`}
                        style={{ alignSelf: "flex-start" }}
                      >
                        <Text
                ...
                {displayCourse?.finalMark && (
                  ...
                  >
                    {showFinalMark ? (
                      <View
                        className={`bg-success/90 rounded-xl px-3 py-1`}
                        style={{ alignSelf: "flex-start" }}
                      >
                        <Text
                          className={`${isDark ? "text-appblack" : "text-appwhite"} text-sm font-medium`}
  ...
  return (
    <>
      ...
      <View className="">
        <LiquidGlassView
          className="rounded-xl p-6 w-full flex-row items-center justify-between"
          fallbackBackgroundColor={activeTone.bg3}
          glassTintColor={activeTone.bg2}
          glassEffectStyle="clear"
        >
          ...
          <View className={`flex-row justify-end items-center flex-shrink-0`}>
            <View>
              {(() => {
                ...
                const courseRingColor =
                  mark < 50
                    ? DEFAULT_STATUS_COLORS.danger
                    : themePresetId === "default"
                      ? DEFAULT_STATUS_COLORS.success
                      : activeTone.accent;
...
export const QuickCourse = ({
  ...
  if (isLoading && !selectedCourse) {
    return (
      <LiquidGlassView
        className="rounded-xl p-6  w-full items-center py-14"
        fallbackBackgroundColor={activeTone.bg3}
        glassTintColor={activeTone.bg2}
        glassEffectStyle="clear"
      >
  ...
  if (!selectedCourse) {
    return (
      <LiquidGlassView
        className="rounded-xl p-6 mb-6  w-full"
        fallbackBackgroundColor={activeTone.bg3}
        glassTintColor={activeTone.bg2}
        glassEffectStyle="clear"
      >
